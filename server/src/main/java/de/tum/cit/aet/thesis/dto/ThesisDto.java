...
public record ThesisDto(
  ...
  public static ThesisDto fromThesisEntity(Thesis thesis, boolean advisorAccess,
    ...
    for (int i = 0; i < stateChanges.size(); i++) {
      ...
      Instant endedAt =
          i + 1 < stateChanges.size() ? stateChanges.get(i + 1).getChangedAt() : Instant.now();

      if (stateChange.getId().getState() == ThesisState.FINISHED
          || stateChange.getId().getState() == ThesisState.DROPPED_OUT) {
  ...
  public record ThesisStateChangeDto(
    ...
    public static ThesisStateChangeDto fromStateChangeEntity(ThesisStateChange stateChange,
        Instant endedAt) {
      if (stateChange == null) {
        return null;
      }

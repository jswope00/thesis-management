...
public class Thesis {
  ...
  public boolean hasStudentAccess(User user) {
    ...
    for (ThesisRole role : roles) {
      if (role.getId().getRole().equals(ThesisRoleName.STUDENT) && role.getUser().getId()
          .equals(user.getId())) {
        return true;
      }
    }
  ...
  public boolean hasReadAccess(User user) {
    ...
    return visibility.equals(ThesisVisibility.STUDENT) && user.hasAnyGroup("student", "advisor",
        "supervisor");
  }
